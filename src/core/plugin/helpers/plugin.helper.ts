...
export abstract class PluginHelper {
	public static validateSyntaxBlock(rule: PluginRule): boolean {
	...
	public static getRuleName(node: PluginRule): string {
		return PluginHelper.isRule(node) ? node.selector : node.name;
	}

	public static getRuleParams(node: PluginRule): Nullable<string> {
