...
describe('stylelint-scss', () => {
	...
	describe('scss/at-else-empty-line-before', () => {
		const ruleName = 'scss/at-else-empty-line-before';

		describe('invalid', () => {
			it('should report when there is an empty line before @else', async () => {
				const result = await lint({
					code: `
            @if ($x == 1) {
            }
          ...
          `,
				...
				const warning = getWarning(result, ruleName);
				expect(warning.text).toBe(`Unexpected empty line before @else (scss/at-else-empty-line-before)`);
			});

			it('should report when there is an empty line before @else following @else if', async () => {
				const result = await lint({
					code: `
            @if ($x == 1) {
            } @else if ($x == 2) {
		...
		describe('valid', () => {
			it('should not report when there is no empty line before @else', async () => {
				const result = await lint({
					code: `
            @if ($x == 1) {
            } @else if ($x == 2) {
          ...
          `,
			...
			it('should not report when @else is used in different context', async () => {
				const result = await lint({
					code: `
            a {
              @if ($x == 1) {
